# -*- coding: utf-8 -*-
# =====================================================================
#  Neiry -> головной TouchDesigner (Эфир) : ридер бенда стены L
#  ЭКЗЕМПЛЯР 3 из 3 — все три бенда на ОДНОЙ машине, по процессу на бенд:
...
#
#  Бенд:    EF:21:DE:1C:A3:67  (вшит; env НЕ переопределяет — см. комментарий у PORT)
#  Метрики: UDP JSON -> NEIRY_HEAD:9001   (контракт художницы, НЕ менять)
#  События: UDP JSON -> NEIRY_HEAD:9011   {"event": ..., "band": "L", ...}
#
#  Межпроцессная дисциплина:
#   - лок экземпляра: localhost:47656 (дубль выходит сам)
...
#   - сдвиг старта 20с, чтобы после ребута три процесса не ломились хором
#   - heartbeat: logs/neiry_heartbeat_3.txt (~1/с, его ждёт NEURWATCH)
#
...
#   - faulthandler -> logs/crash_3.log; почасовые логи logs/reader_3/.
#
#  Запуск: START_NEIRY_BAND_3.bat (задача NEIRYSTART3, onlogon)
# =====================================================================
...
BAND_LABEL = 'L'   # стена: R / C / L
BAND_ADDR = 'EF:21:DE:1C:A3:67'     # свой бенд этого экземпляра
INSTANCE = 3           # номер экземпляра 1..3
START_DELAY = 20    # сдвиг старта, с
LOCK_PORT = 47656   # лок «один экземпляр» (47653 + INSTANCE)
...
RECALIB_AFTER_SEC = 180  # снят дольше -> считаем сменой человека, калибровка заново
                         # (короче — тот же человек, базовую линию сохраняем)
...
_worn = {'state': None, 'cand': None, 'cand_t': 0.0, 'off_since': 0.0}   # None = ещё неизвестно
_recalib = {'need': False}   # выставляется при надевании после долгого снятия
...
def _worn_update(has_contact, source):
    ...
    if now - _worn['cand_t'] >= need and _worn['state'] != cand:
        ...
        if cand:
            off_for = (now - _worn['off_since']) if _worn['off_since'] else 0
            log('ОБОДОК НАДЕТ (%s, был снят %dс)' % (source, off_for))
            send_event('band_worn', source=source, off_sec=int(off_for), resist_kohm=kohm)
            # долго снят -> вероятно другой человек, нужна калибровка заново
            if calib_done and off_for >= RECALIB_AFTER_SEC:
                _recalib['need'] = True
        else:
            _worn['off_since'] = now
...
def session():
    ...
    _worn.update(state=None, cand=None, cand_t=0.0, off_since=0.0)
    _recalib['need'] = False
    ...
    if started:
        ...
        while connected:
            ...
            if not calib_done and time.time() - _calib_prog['t'] > CALIB_STALL_SEC:
                ...
                break
            if _recalib['need']:
                # надет после долгого снятия -> вероятно другой человек:
                # пересобираем EmotionalMath (чужую базовую линию не тащим)
                _recalib['need'] = False
                log('ободок надет после долгого снятия — калибровка заново (новый человек?)')
                send_event('recalibrating')
                calib_done = False; _last_calib = -1; _last_art = None
                _calib_prog['t'] = time.time()
                math = build_math()
        else:
