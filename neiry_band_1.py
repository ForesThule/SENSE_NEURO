# -*- coding: utf-8 -*-
# =====================================================================
#  Neiry -> головной TouchDesigner (Эфир) : ридер бенда стены R
#  ЭКЗЕМПЛЯР 1 из 3 — все три бенда на ОДНОЙ машине, по процессу на бенд:
...
#
#  Бенд:    F7:14:0E:FE:9D:20  (вшит; env НЕ переопределяет — см. комментарий у PORT)
#  Метрики: UDP JSON -> NEIRY_HEAD:9003   (контракт художницы, НЕ менять)
#  События: UDP JSON -> NEIRY_HEAD:9013   {"event": ..., "band": "R", ...}
#
#  Межпроцессная дисциплина:
#   - лок экземпляра: localhost:47654 (дубль выходит сам)
...
#   - сдвиг старта 0с, чтобы после ребута три процесса не ломились хором
#   - heartbeat: logs/neiry_heartbeat_1.txt (~1/с, его ждёт NEURWATCH)
#
...
#   - faulthandler -> logs/crash_1.log; почасовые логи logs/reader_1/.
#
#  Запуск: START_NEIRY_BAND_1.bat (задача NEIRYSTART1, onlogon)
# =====================================================================
...
BAND_LABEL = 'R'   # стена: R / C / L
BAND_ADDR = 'F7:14:0E:FE:9D:20'     # свой бенд этого экземпляра
INSTANCE = 1           # номер экземпляра 1..3
START_DELAY = 0    # сдвиг старта, с
LOCK_PORT = 47654   # лок «один экземпляр» (47653 + INSTANCE)
