...
RSSI_MIN = int(os.environ.get('NEIRY_RSSI_MIN', -85))  # слабее — не подключаемся:
                      # каждое подключение на слабом сигнале — бросок костей,
                      # нативный create_sensor падает с access violation при
                      # обрыве в момент коннекта; на площадке можно ужесточить
                      # (напр. -78) через env в .bat
